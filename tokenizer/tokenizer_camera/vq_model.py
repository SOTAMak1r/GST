...
class VQModel(nn.Module):
    ...
    def loss_function(self, input, output, diff):
        recons_loss = F.mse_loss(output, input)
        vq_loss     = diff[0] + diff[1] + diff[2]
...
class Encoder(nn.Module):
    def __init__(self, in_channels=6, ch=128, ch_mult=(1,1,2,2,4), num_res_blocks=2, 
...
class Decoder(nn.Module):
    def __init__(self, z_channels=256, ch=128, ch_mult=(1,1,2,2,4), num_res_blocks=2, norm_type="group",
                 dropout=0.0, resamp_with_conv=True, out_channels=6):
        super().__init__()
        self.num_resolutions = len(ch_mult)
        self.num_res_blocks = num_res_blocks
...
class VectorQuantizer(nn.Module):
    def __init__(self, n_e, e_dim, beta, entropy_loss_ratio, l2_norm, show_usage):
        ...
        if self.show_usage:
            # self.register_buffer("codebook_used", nn.Parameter(torch.zeros(65536 * 4)))
    ...
    def forward(self, z):
        ...
        if self.show_usage and self.training:
            ...
            # print(f'[debug] {min_encoding_indices.shape, self.codebook_used.shape}')
